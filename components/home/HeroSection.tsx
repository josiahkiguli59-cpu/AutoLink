...
import { Search, ShieldCheck, MessageCircle, Sparkles, SlidersHorizontal, ArrowRight } from 'lucide-react';
import { POPULAR_BRANDS, BODY_TYPES } from '@/lib/mock-data';

export default function HeroSection() {
  const router = useRouter();
  const [activeTab, setActiveTab] = useState<'All' | 'Brand New' | 'Foreign Used' | 'Local Used'>('All');
    ...
    <section className="relative min-h-[90vh] flex items-center justify-center pt-20 pb-24 overflow-hidden">
      ...
      <div className="relative z-10 mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 text-center">
        ...
        <p className="mt-5 text-base sm:text-lg text-slate-300 max-w-2xl mx-auto leading-relaxed font-normal">
          Browse verified foreign-used, brand-new, and local luxury cars in Kampala. Compare specs side-by-side and chat directly with dealers on WhatsApp.
        </p>
        ...
        <div className="mt-10 max-w-4xl mx-auto rounded-3xl border border-border/80 bg-surface-300/95 p-3 sm:p-5 shadow-2xl shadow-black/80 backdrop-blur-2xl">
          ...
          <div className="flex items-center gap-1.5 overflow-x-auto pb-3 mb-3 border-b border-border/60">
            {(['All', 'Brand New', 'Foreign Used', 'Local Used'] as const).map((tab) => (
              <button
                key={tab}
                type="button"
          ...
          <div className="mt-3 pt-3 border-t border-border/50 flex flex-col sm:flex-row items-center justify-between gap-2 text-xs text-slate-400">
            <div className="flex items-center gap-2 w-full sm:w-auto">
              <span className="shrink-0 text-[11px] font-medium text-slate-300">Popular:</span>
              <div className="flex flex-wrap gap-1.5">
                {['Prado TX-L', 'Fortuner', 'GLE 400', 'Hilux GR', 'Premio'].map((keywordItem) => (
                  <button
                    key={keywordItem}
                    type="button"
                    onClick={() => router.push(`/cars?keyword=${encodeURIComponent(keywordItem)}`)}
        ...
        <div className="mt-14 grid grid-cols-2 md:grid-cols-4 gap-4 max-w-4xl mx-auto pt-6 border-t border-border/40">
          <div className="p-3 text-center">
            <div className="text-2xl sm:text-3xl font-black text-white font-display">2,500+</div>
            <div className="text-xs text-slate-400 mt-0.5">Verified Cars Available</div>
          </div>
          <div className="p-3 text-center">
            <div className="text-2xl sm:text-3xl font-black text-gold-400 font-display">200-Point</div>
            <div className="text-xs text-slate-400 mt-0.5">Mechanical Inspection</div>
          </div>
          <div className="p-3 text-center">
            <div className="text-2xl sm:text-3xl font-black text-emerald-400 font-display">Instant</div>
            <div className="text-xs text-slate-400 mt-0.5">Dealer WhatsApp Contact</div>
          </div>
          <div className="p-3 text-center">
            <div className="text-2xl sm:text-3xl font-black text-white font-display">0% Hidden</div>
            <div className="text-xs text-slate-400 mt-0.5">Middleman Broker Fees</div>
          </div>
        </div>
      </div>
    </section>
